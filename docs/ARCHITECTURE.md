...
- src/parser.ts
  ...
  - Internal helper `parseCVPart(rest, chapterVerseSeparators, rangeSeparator, bookId)` extracts chapter/verse numbers from the numeric tail of a match. When `bookId` is in `SINGLE_CHAPTER_BOOKS` and the pattern contains no verse separator (`N` or `N-N`), the numbers are interpreted as verse(s) of chapter 1 (`chapterStart: 1, verseStart: N, verseEnd?: N`). For multi-chapter books the same patterns produce a chapter or chapter-range. Used by both the `buildRefScanner` path and the legacy `parseCzechBibleRef` path (via `parseChapterVersePart`).
  - `formatRef` single-chapter special case: when `ref.bookId` is in `SINGLE_CHAPTER_BOOKS` and `ref.verseStart` is present, the chapter number is omitted and the reference is formatted as `abbr bookChapSep verseStart[-verseEnd]`
- src/types.ts
...
- src/sources/adapters.ts
  ...
  - Registered translation adapters: `"getbible-v2"`, `"beblia-xml"` (Beblia XML GitHub source; implements `listUrl` returning the fixed GitHub Contents API URL `https://api.github.com/repos/Beblia/Holy-Bible-XML-Format/contents/` and `listAvailable` parsing the Contents API response — array of `{ name, type }` items — filtering `.xml` files and mapping each to `RemoteTranslationEntry` with `id` = lowercased stem, `displayName` = stem, `language` = `""`, `remoteId` = full filename; `listAvailable` returns `[]` on parse failure; catalog enrichment of `displayName`/`language` for known entries is delegated to the caller `settingsTab.ts`), `"biblens-data"` (transforms v1 JSON → flat TranslationData; key normalization duplicated from `translationLoader.ts` — sharing via import would violate the module boundary)
