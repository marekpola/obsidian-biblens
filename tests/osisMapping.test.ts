import { describe, it, expect } from "vitest";
import { osisToUsfm } from "../src/osisMapping";

describe("osisToUsfm", () => {
  it('maps "Gen" to "GEN"', () => {
    expect(osisToUsfm("Gen")).toBe("GEN");
  });

  it('maps "Matt" to "MAT"', () => {
    expect(osisToUsfm("Matt")).toBe("MAT");
  });

  it("returns undefined for unknown input", () => {
    expect(osisToUsfm("Unknown")).toBeUndefined();
    expect(osisToUsfm("")).toBeUndefined();
  });

  it("covers all OT canonical books", () => {
    const ot: [string, string][] = [
      ["Exod", "EXO"], ["Lev", "LEV"], ["Num", "NUM"], ["Deut", "DEU"],
      ["Josh", "JOS"], ["Judg", "JDG"], ["Ruth", "RUT"],
      ["1Sam", "1SA"], ["2Sam", "2SA"], ["1Kgs", "1KI"], ["2Kgs", "2KI"],
      ["1Chr", "1CH"], ["2Chr", "2CH"], ["Ezra", "EZR"], ["Neh", "NEH"],
      ["Esth", "EST"], ["Job", "JOB"], ["Ps", "PSA"], ["Prov", "PRO"],
      ["Eccl", "ECC"], ["Song", "SNG"], ["Isa", "ISA"], ["Jer", "JER"],
      ["Lam", "LAM"], ["Ezek", "EZK"], ["Dan", "DAN"], ["Hos", "HOS"],
      ["Joel", "JOL"], ["Amos", "AMO"], ["Obad", "OBA"], ["Jonah", "JON"],
      ["Mic", "MIC"], ["Nah", "NAM"], ["Hab", "HAB"], ["Zeph", "ZEP"],
      ["Hag", "HAG"], ["Zech", "ZEC"], ["Mal", "MAL"],
    ];
    for (const [osis, usfm] of ot) {
      expect(osisToUsfm(osis), `expected ${osis} → ${usfm}`).toBe(usfm);
    }
  });

  it("covers all NT canonical books", () => {
    const nt: [string, string][] = [
      ["Mark", "MRK"], ["Luke", "LUK"], ["John", "JHN"], ["Acts", "ACT"],
      ["Rom", "ROM"], ["1Cor", "1CO"], ["2Cor", "2CO"], ["Gal", "GAL"],
      ["Eph", "EPH"], ["Phil", "PHP"], ["Col", "COL"],
      ["1Thess", "1TH"], ["2Thess", "2TH"], ["1Tim", "1TI"], ["2Tim", "2TI"],
      ["Titus", "TIT"], ["Phlm", "PHM"], ["Heb", "HEB"], ["Jas", "JAS"],
      ["1Pet", "1PE"], ["2Pet", "2PE"], ["1John", "1JN"], ["2John", "2JN"],
      ["3John", "3JN"], ["Jude", "JUD"], ["Rev", "REV"],
    ];
    for (const [osis, usfm] of nt) {
      expect(osisToUsfm(osis), `expected ${osis} → ${usfm}`).toBe(usfm);
    }
  });

  it("covers deuterocanonical books", () => {
    const dc: [string, string][] = [
      ["Tob", "TOB"], ["Jdt", "JDT"], ["EsthGr", "ESG"], ["AddEsth", "ESG"],
      ["Wis", "WIS"], ["Sir", "SIR"], ["Bar", "BAR"], ["EpJer", "LJE"],
      ["PrAzar", "S3Y"], ["Sus", "SUS"], ["Bel", "BEL"],
      ["1Macc", "1MA"], ["2Macc", "2MA"], ["3Macc", "3MA"], ["4Macc", "4MA"],
      ["1Esd", "1ES"], ["2Esd", "2ES"], ["PrMan", "MAN"],
    ];
    for (const [osis, usfm] of dc) {
      expect(osisToUsfm(osis), `expected ${osis} → ${usfm}`).toBe(usfm);
    }
  });
});
